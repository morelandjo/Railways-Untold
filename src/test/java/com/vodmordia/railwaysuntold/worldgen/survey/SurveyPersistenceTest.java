...
class SurveyPersistenceTest {
    ...
    private static final class FakeExtractor implements SurveyExtractor<FakeData> {
        ...
        @Override public FakeData fromNbt(CompoundTag tag, HolderLookup.Provider provider) {
    ...
    void storedSurveyRoundTripsThroughRegistry() {
        ...
        // FakeData ignores the provider, so null is fine for this pure round-trip.
        CompoundTag tag = stored.toNbt(null);
        StoredSurvey parsed = StoredSurvey.fromNbt(tag, null);
    ...
    void unregisteredExtractorEntryIsSkippedOnLoad() {
        ...
        CompoundTag tag = new StoredSurvey(
                ...
                new SurveyResult(Map.of("fake", new FakeData(1)))).toNbt(null);
        SurveyExtractorRegistry.clear();
