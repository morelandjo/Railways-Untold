...
public class CreateTrainUtils {
    ...
    // Cached for network sending
    private static volatile Object catnipNetwork = null;
    private static volatile Method sendToAllClientsMethod = null;
    ...
    /**
     * Initializes the reflection to access CatnipServices.NETWORK.
     */
    private static void initializeNetworkReflection() {
        ...
        synchronized (NETWORK_REFLECTION_LOCK) {
            ...
            try {
                // Try to get CatnipServices.NETWORK
                Class<?> catnipServicesClass = Class.forName("net.createmod.catnip.platform.CatnipServices");
                Object networkField = catnipServicesClass.getField(CreateReflectionFields.NETWORK).get(null);
                catnipNetwork = networkField;

                // Find the sendToAllClients method
                for (Method method : networkField.getClass().getMethods()) {
                    if (method.getName().equals("sendToAllClients") && method.getParameterCount() == 1) {
                        sendToAllClientsMethod = method;
                        break;
                    }
                }

                if (sendToAllClientsMethod == null) {
                    LOGGER.warn("[CREATE-TRAIN] Could not find sendToAllClients method");
                }
            } catch (ReflectiveOperationException | RuntimeException e) {
                LOGGER.error("[CREATE-TRAIN] Failed to initialize CatnipServices reflection: {}", e.getMessage());
            } finally {
    ...
    private static void initializeBogeyReflection() {
        ...
        synchronized (BOGEY_REFLECTION_LOCK) {
            ...
            try {
                ...
                // BogeyStyle has an 'id' field of type ResourceLocation
                Field idField = bogeyStyle.getClass().getField("id");
                cachedStandardStyleId = (ResourceLocation) idField.get(bogeyStyle);
            } catch (ReflectiveOperationException | RuntimeException e) {
                LOGGER.error("[CREATE-TRAIN] Failed to get STANDARD bogey style via reflection: {}", e.getMessage());
                // Default fallback
                cachedStandardStyleId = ResourceLocation.fromNamespaceAndPath("create", "standard");
            }
    ...
    /**
     * Sends the AddTrainPacket to all clients to sync the new train.
     *
    ...
    public static void syncTrainToClients(Train train) {
        ...
        if (catnipNetwork != null && sendToAllClientsMethod != null) {
            try {
                AddTrainPacket packet = new AddTrainPacket(train);
                sendToAllClientsMethod.invoke(catnipNetwork, packet);
            } catch (ReflectiveOperationException | RuntimeException e) {
                LOGGER.error("[CREATE-TRAIN] Failed to send AddTrainPacket: {}", e.getMessage());
            }
        } else {
            LOGGER.warn("[CREATE-TRAIN] Cannot send AddTrainPacket - CatnipServices not available");
        }
    }
