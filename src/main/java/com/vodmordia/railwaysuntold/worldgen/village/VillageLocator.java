...
public class VillageLocator {
    ...
    /**
     * Reads the GROUND-TRUTH placed village layout from already-loaded chunks: the real
     * {@link StructureStart} piece boxes, not the {@code structure.generate} prediction. Picks the
     * village start nearest {@code near}. Returns empty if no village start is present in the loaded
     * region. Used by the survey StructureExtractor to produce an exact layout.
     */
        ...
        try {
            ...
            level.structureManager().startsForStructure(chunk, structure ->
                ...
                if (structureStart.isValid()) {
                    // Find which resolved target this structure matches for ID generation
