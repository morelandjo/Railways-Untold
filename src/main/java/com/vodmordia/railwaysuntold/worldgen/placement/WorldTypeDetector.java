...
public class WorldTypeDetector {
    ...
    static {
        try {
            CODEC_METHOD = ChunkGenerator.class.getDeclaredMethod("codec");
            CODEC_METHOD.setAccessible(true);
        } catch (NoSuchMethodException e) {
            throw new RuntimeException("Failed to find ChunkGenerator.codec() method", e);
        }
    }
    ...
    public static String detectWorldType(ServerLevel level) {
        ...
        try {
            MapCodec<? extends ChunkGenerator> codec =
                    (MapCodec<? extends ChunkGenerator>) CODEC_METHOD.invoke(generator);
