 ...
 * or track on one edge and a dead end marker on the opposite edge.
 *
...
public class SchematicValidator {
    ...
    /**
     * Represents a track position within the schematic.
     */
    private record TrackPosition(int x, int y, int z) {
    }

    /**
     * Represents a start block position with its facing direction.
     */
    private record StartPosition(int x, int y, int z, Direction facing) {
    }

    /**
     * Validates a loaded schematic for station use.
     *
     * @param schematic The loaded schematic to validate
     * @return Validation result with track info if valid, or error message if invalid
     */
    public static SchematicValidationResult validate(NbtSchematicLoader.LoadedSchematic schematic) {
        ...
        // Combine dead end and start block positions as generic "marker" positions for alignment
        ...
        // Validate Y level consistency across track, dead end, and start blocks
    ...
    private static TrackAlignmentResult determineTrackAlignment(List<TrackPosition> trackPositions,
        ...
        // Combine track and dead end positions for alignment detection
        List<TrackPosition> allPositions = new ArrayList<>(trackPositions);
        allPositions.addAll(deadEndPositions);
        ...
        // Prefer Z-axis (runs along Z) if blocks share X, else X-axis
    ...
    private static SchematicValidationResult validateZAxisTrack(List<TrackPosition> trackPositions,
        ...
        // Normal case: track on both edges
        ...
        // Dead-end case: track on one edge, dead end on the other
        ...
        // Start block case: track on one edge, start block on the other
    ...
    /**
     * Finds all track blocks in the schematic.
     */
    ...
    /**
     * Finds all start marker blocks in the schematic, capturing their facing direction.
     */
    ...
    /**
     * Finds all dead end marker blocks in the schematic.
     */
