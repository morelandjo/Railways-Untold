...
public class StraightTrackPlacer {
     ...
     *
     * Track blocks are placed sequentially along the path. Create's TrackPropagator
     * will automatically discover adjacent tracks and connect them as straight edges.
     *
     ...
     */
    /**
     * Places a straight track segment without head ownership tracking.
     */
    public static StraightPlacementResult place(ServerLevel level, BlockPos start, BlockPos end, Direction direction) {
        return place(level, start, end, direction, null, 0);
    }

    /**
     * Places a straight track segment with head ownership tracking.
     */
    public static StraightPlacementResult place(ServerLevel level, BlockPos start, BlockPos end, Direction direction, UUID headId) {
        return place(level, start, end, direction, headId, 0);
    }

    /**
     * Places a straight track segment with head ownership tracking and torch offset.
     *
     * @param torchOffset Starting offset for torch interval calculation in tunnel decoration
     */
    public static StraightPlacementResult place(ServerLevel level, BlockPos start, BlockPos end, Direction direction, UUID headId, int torchOffset) {
        ...
        ConnectedBoundaryTracker.confirmSegment(level, start, current,
                ConnectionType.STRAIGHT, placedPositions, headId);
