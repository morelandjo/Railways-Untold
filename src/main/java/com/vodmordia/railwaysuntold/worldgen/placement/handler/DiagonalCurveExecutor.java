...
public class DiagonalCurveExecutor extends AbstractPlacementExecutor {
    ...
    public HandlerResult handle(PlacementContext ctx) {
        ...
        try {
            ...
            ConnectedBoundaryTracker.confirmSegment(ctx.level(), start, end, ConnectionType.BEZIER, null, ctx.head().getHeadId());
     ...
     * via {@link ChunkLoadTrackExpander#holdWorkingChunks}, so chunky's organic tickets
     * can drop without unloading what the head is about to need.
     *
     */
