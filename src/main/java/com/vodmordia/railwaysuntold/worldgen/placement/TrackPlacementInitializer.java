...
public class TrackPlacementInitializer {
    ...
    static {
        ...
        SystemStateManager.register("ServerTickHandler", ServerTickHandler::clearDelayedTasks);
    ...
    public static void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        level.getServer().execute(() -> {
            try {
                ...
                InitialChunkSelector selector =
                        new InitialChunkSelector(
                                level.getSeed(), worldSpawn);
                net.minecraft.world.level.ChunkPos initialChunk = selector.getInitialChunk();
                net.minecraft.core.Direction trackDirection = selector.getInitialDirection();
    ...
    /**
     * Clears all static state from all systems.
     * Called during server start/stop to ensure clean state.
     */
     ...
     */
    private static net.minecraft.world.level.ChunkPos avoidNearbyVillage(
            ServerLevel level, net.minecraft.world.level.ChunkPos initialChunk,
            net.minecraft.core.Direction trackDirection) {
        // Search radius in blocks - covers villages up to ~5 chunks away
        int searchRadius = 80;

        net.minecraft.core.BlockPos origin = initialChunk.getWorldPosition().offset(8, 0, 8);
        List<net.minecraft.world.level.levelgen.structure.BoundingBox> footprints =
                VillageLocator.placedVillageFootprintsInRadius(level, origin, searchRadius);

        net.minecraft.core.BlockPos nudged = StationOriginNudge.resolve(origin, trackDirection, footprints,
        ...
        return new net.minecraft.world.level.ChunkPos(nudged);
    }
