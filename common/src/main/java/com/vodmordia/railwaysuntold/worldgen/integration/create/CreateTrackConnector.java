    ...
    private static TrackBlockPair getValidatedTrackBlocks(ServerLevel level, BlockPos firstPos, BlockPos secondPos) {
        ...
        if (firstState == null || secondState == null) {
            if (RailwaysUntoldConfig.isVerboseLoggingEnabled()) {
                LOGGER.warn("[TRACK-CONNECT] getValidatedTrackBlocks: null block state at {} or {}", firstPos, secondPos);
            }
            return null;
        }
        ...
        if (iTrackBlockClass == null ||
            ...
            if (RailwaysUntoldConfig.isVerboseLoggingEnabled()) {
                LOGGER.warn("[TRACK-CONNECT] getValidatedTrackBlocks: not track blocks at {} or {} (class={})", firstPos, secondPos, iTrackBlockClass);
            }
            return null;
        }
