...
public class EventPlacementRule implements PlacementRule {
    ...
    public Optional<PlacementDecision> decide(DeciderContext context) {
        ...
        if (context.head().getVillageState().hasReachedVillage()) {
            LOGGER.debug("[EVENT-RULE] Skipped: has reached village");
            return Optional.empty();
        }
    ...
    private int getFootprintLength(EventDefinitionLoader.ValidatedEventEntry event, Direction dir) {
        if (event instanceof EventDefinitionLoader.ValidatedEventEntry.NbtEntry nbt) {
            if (dir == Direction.NORTH || dir == Direction.SOUTH) {
                return nbt.schematic().getLength();
            }
            return nbt.schematic().getWidth();
        }
        if (event instanceof EventDefinitionLoader.ValidatedEventEntry.JigsawEntry jigsaw) {
            return jigsaw.definition().estimatedFootprint();
        }
        return EventDefinition.DEFAULT_JIGSAW_FOOTPRINT;
    }
