...
public final class StructureExtractor implements SurveyExtractor<SurveyedStructureLayout> {
    ...
    public SurveyedStructureLayout extract(SurveyContext ctx) {
        BlockPos near = ctx.region().anchor().getMiddleBlockPosition(0);
    ...
    @Override
    public SurveyedStructureLayout fromNbt(CompoundTag tag) {
        return SurveyedStructureLayout.fromNbt(tag);
    }
}
